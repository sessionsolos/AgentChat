 ...
 * POST /api/schools
 *
 * Request body:  SchoolsRequest  { state?, ids?, incomeBand?, majors?, includeOutOfState? }
 *   At least one of `state` or `ids` must be provided (enforced by SchoolsRequestSchema refine).
 ...
 *
 * When `ids` is present, delegates to fetchSchoolsByIds; otherwise uses the
 * state-based fetchSchoolCosts. Both functions fall back to cache internally
 * on any API failure — this handler never returns 500 on a data-source failure.
 */
...
import {
  fetchSchoolCosts,
  fetchSchoolsByIds,
} from "@/lib/datasources/scorecard";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    if (parsedRequest.ids && parsedRequest.ids.length > 0) {
      // id-based lookup
      schools = await fetchSchoolsByIds(parsedRequest.ids);
    } else {
      // state-based lookup
    ...
    dataSource =
      schools.length > 0 && schools[0].dataSource === "live"
        ? "live"
        : "cached";
  } catch {
    // Belt-and-suspenders: the fetch functions should never throw, but if they
    // do we serve the full cache rather than returning 500.
    if (parsedRequest.ids && parsedRequest.ids.length > 0) {
      schools = (cachedSchools as unknown as SchoolCost[]).filter((s) =>
        parsedRequest.ids!.includes(s.id)
      );
    } else {
      schools = (cachedSchools as unknown as SchoolCost[]).filter(
        (s) =>
          s.state === parsedRequest.state || parsedRequest.includeOutOfState
      );
    }
    dataSource = "cached";
  }
